...
For binary strings, "similar" means a small **Hamming distance** - the number of bits that differ:
...
That dashed circle is the **descriptor patch** - the only region compared.^[The patch is a $31\times31$ *square*, but only the inscribed **circle** is effectively used - rotating the sampling pattern keeps the same pixels only for a circular region, which is why the demo draws a circle.] Since the descriptor is centred on the corner and rotation-invariant, the match holds as you move and rotate it - until you drag it near the circle. Once that intrudes on the patch, the bits flip and the **match breaks**.
